...
#SBATCH --output=/scratch/gautschi/shin283/paper/others/books/pdf2md/logs/%j_pdf2md.out
#SBATCH --error=/scratch/gautschi/shin283/paper/others/books/pdf2md/logs/%j_pdf2md.err
#SBATCH --exclude=h000,h003

# ============================================
# Batch PDF to Markdown Conversion
# Books dataset (~84 PDFs in subdirectories)
# ============================================

echo "========================================="
echo "PDF to Markdown Batch Conversion - Books"
...
echo ""
echo "Starting Marker batch conversion for Books dataset..."
echo ""
...
# Get all PDFs recursively from all subdirectories
all_pdfs = sorted(input_dir.rglob("*.pdf"))
# Filter out any PDFs in the pdf2md output directory
pdfs = [p for p in all_pdfs if "pdf2md" not in str(p)]
total = len(pdfs)
print(f"Processing {total} PDFs from Books dataset (including subdirectories)")
sys.stdout.flush()
...
for i, pdf_path in enumerate(pdfs):
    # Create output subdir that preserves relative path structure
    rel_path = pdf_path.relative_to(input_dir)
    # Use parent directories + stem as output folder name (flattened with underscores)
...
print("")
print("=" * 50)
print(f"Books Conversion completed in {elapsed/60:.2f} minutes")
...
if error_list:
    import json
    error_file = output_dir / "errors_books.json"
...
echo ""
echo "========================================="
echo "Books Conversion completed"
echo "End time: $(date)"
